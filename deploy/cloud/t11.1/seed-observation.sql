...
begin
  ...
  observe('sprites',q'[select json_object(* returning clob) from doom_asset where asset_kind='sprite_patch' order by asset_name]');
  observe('sounds',q'[select json_object(* returning clob) from doom_asset where asset_kind='sound' order by asset_name]');
  observe('music',q'[select json_object(* returning clob) from doom_asset where asset_kind='music' order by asset_name]');
