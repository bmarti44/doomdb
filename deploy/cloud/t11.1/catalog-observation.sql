...
begin
  ...
  -- CLOUD_SERVICE is populated by Autonomous Database itself. Do not grant
  -- the application schema EXECUTE on DBMS_CLOUD merely to make that SYS
  -- package visible through ALL_OBJECTS.
  ...
  select count(*) into l_unexpected from user_tab_privs
   where grantee='PUBLIC'
     and not (
       (table_name='DOOM_API' and privilege='EXECUTE')
       or (table_name='PUBLIC_HEALTH' and privilege='SELECT')
       or (table_name=sys_context('USERENV','CURRENT_SCHEMA')
  ...
  if l_mle_modules<>1 or l_mle_envs<>1 or l_mle_specs<>27 or
     l_source_bytes<>1181281 or
     l_source_sha<>'c613bb5106d6572d1023ae6caf9045f52d493005bc1be001326acd3826d8eae1' or
...
begin
  for r in (
    select table_name from user_tab_privs where grantee='PUBLIC'
      and ((table_name='DOOM_API' and privilege='EXECUTE')
        or (table_name='PUBLIC_HEALTH' and privilege='SELECT'))
      order by table_name
  ) loop
    dbms_output.put_line('T111_PUBLIC_EXECUTE|'||trim(r.table_name));
  end loop;
  for r in (
    select parsing_object,type,status from user_ords_enabled_objects
      order by parsing_object
  ) loop
    dbms_output.put_line('T111_REST|'||trim(r.parsing_object)||'|'||
