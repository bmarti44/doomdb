...
begin
  ...
  dbms_output.put_line(
    'PMLE_FREE_LIVE_HYBRID_LOAD|PASS|pack_bytes='||l_bytes);
end;
/
...
begin
  ...
  dbms_output.put_line(
    'PMLE_FREE_LIVE_HYBRID_TEXTURE|PASS|bytes='||l_bytes||
    '|sha256='||l_actual_sha);
end;
/
...
declare
  ...
  l_generated_a raw(32000);l_generated_b raw(32000);
  l_hybrid_a raw(32000);l_hybrid_b raw(32000);
  l_value number;l_stats varchar2(4000);
begin
  for index_ in 1..l_poses.count loop
    l_value:=doom_free_gen_frame(l_poses(index_));
    l_generated_a:=doom_free_gen_frame_chunk(0,32000);
    l_generated_b:=doom_free_gen_frame_chunk(32000,32000);
    l_value:=doom_free_hybrid_frame(l_poses(index_));
    l_hybrid_a:=doom_free_hybrid_frame_chunk(0,32000);
    l_hybrid_b:=doom_free_hybrid_frame_chunk(32000,32000);
    l_stats:=doom_free_hybrid_stats;
    if l_generated_a<>l_hybrid_a or l_generated_b<>l_hybrid_b then
      raise_application_error(
        -20796,'hybrid frame mismatch at pose '||l_poses(index_));
    end if;
    dbms_output.put_line(
      'PMLE_FREE_LIVE_HYBRID_EQUIVALENCE|PASS|pose='||l_poses(index_)||
      '|frame_bytes=64000|stats='||l_stats);
  end loop;
end;
/
...
begin
  ...
  for pass in 1..c_passes loop
    ...
    for frame_ in 1..c_frames loop
      ...
      l_checksum:=l_checksum+doom_free_hybrid_frame(c_start+frame_-1);
    ...
    l_stats:=doom_free_hybrid_stats;
    dbms_output.put_line(
      'PMLE_FREE_LIVE_HYBRID_PASS|PASS|pass='||pass||
      '|frames='||c_frames||
