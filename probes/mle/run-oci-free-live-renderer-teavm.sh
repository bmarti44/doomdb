...
raster="${PMLE_FREE_LIVE_TEAVM_RASTER:-NO}"
benchmark="$probe/benchmark-oci-free-live-renderer-teavm.sql"
pass_marker='PMLE_FREE_LIVE_TEAVM_PASS'
if [[ "$raster" == YES ]]; then
  benchmark="$probe/benchmark-oci-free-live-renderer-teavm-raster.sql"
  pass_marker='PMLE_FREE_LIVE_TEAVM_RASTER_PASS'
elif [[ "$raster" != NO ]]; then
...
"$root/scripts/adb-doom-sql.sh" \
  "$benchmark" | tee "$rank_log"
[[ "$(grep -c "^$pass_marker|PASS|" "$rank_log")" == 12 ]]
final_p95="$(awk -F'[=|]' '
  $1==marker && $2=="PASS" {
    pass=0;p95=0
  ...
  marker="$pass_marker" "$rank_log")"
if [[ "$raster" == YES ]]; then
  verdict="$(awk -v p95="$final_p95" 'BEGIN{
    if(p95<=11.330)print "PROMOTE_GENERATED_FULL_WALL_RASTER";
    else if(p95>=20)print "REJECT_GENERATED_FULL_WALL_RASTER";
    else print "RECOVER_GENERATED_RASTER_BUDGET";
  }')"
  kernel=AUTHENTIC_PORTAL_WALL_RASTER
else
  verdict="$(awk -v p95="$final_p95" 'BEGIN{
  ...
  kernel=GEOMETRY_ONLY
fi
printf 'PMLE_FREE_LIVE_TEAVM_VERDICT|%s|final_two_worst_p95_ms=%s|kernel=%s|classification=DIAGNOSTIC_NOT_GATE\n' \
  "$verdict" "$final_p95" "$kernel" | tee "$verdict_log"
