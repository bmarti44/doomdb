...
declare
  ...
  l_j pls_integer;
begin
  for i in 1..c_warmups loop doom_mle_hybrid_draw_calls(i, l_checksum); end loop;
  for i in 1..c_samples loop
    l_started := dbms_utility.get_cpu_time;
    doom_mle_hybrid_draw_calls(i, l_checksum);
  ...
  dbms_output.put_line('PMLE_FFI_DRAW|calls_per_frame=2085|draw_calls=2083' ||
