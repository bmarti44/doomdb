...
declare
  ...
  begin
    ...
    else raise_application_error(-20796,'unknown command load kind');end if;
    while l_offset<p_bytes loop
      ...
      if l_loaded<>l_offset then
        raise_application_error(-20796,'short command load '||p_kind);
      end if;
    end loop;
    ...
    if l_loaded<>p_bytes then
      raise_application_error(-20796,'command finalize mismatch '||p_kind);
    end if;
  end;
begin
  ...
  if instr(l_state,'state=multiplayer-initialized|gametic=0|')=0 then
    raise_application_error(-20796,'command initialization mismatch');
  end if;
  dbms_output.put_line('PMLE_OCI_DVL2_COMMAND_FLOOR_LOAD|PASS');
end;
/
...
declare
  ...
  l_done timestamp with time zone;l_command raw(32);l_tic number;
...
begin
  for frame in 1..c_warm+c_frames loop
    ...
    l_tic:=doom_fused_step_commands(2,3,l_command,0);
    l_done:=systimestamp;
    if l_tic<>frame then
      raise_application_error(-20796,'command floor frontier mismatch');
    end if;
  ...
  dbms_output.put_line(
    'PMLE_OCI_DVL2_COMMAND_FLOOR|PASS|frames='||c_frames||
    ...
    '|headroom_to_22_6_ms='||
      to_char(22.6-l_sorted(ceil(c_frames*.95)),'FM9999990.000')||
    '|verdict='||case
      when l_sorted(ceil(c_frames*.95))<=15 then 'BUILD_COMMAND_RASTER'
      when l_sorted(ceil(c_frames*.95))>22.6 then 'REJECT_COMMAND_RASTER'
      else 'MARGINAL' end);
  doom_fused_release;
