...
competing_gate="$(ps ax -o command= | awk '
  /[r]un-ledger-differential|[r]un-decps-ledger|[b]uild-ledger-differential|[r]un-worker-soak|[r]un-differential[.]sh|[r]un-decps-rank-mle|[r]un-presentation-decps-rank/ {print}
')"
[[ -z "$competing_gate" ]] || {
  printf 'MLE worker cutover refuses a competing evidence gate:\n%s\n' \
    "$competing_gate" >&2
  exit 1
}
