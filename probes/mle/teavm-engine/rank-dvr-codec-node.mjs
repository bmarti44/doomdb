...
if (![presentationPath, codecPath, fixturePath, iwadPath, tablePath]
    .every(Boolean)) {
  throw new Error(
    'usage: rank-dvr-codec-node.mjs PRESENTATION CODEC FIXTURE IWAD TABLES',
  );
}
...
for (let index = 0; index < rows.length; index += 1) {
  ...
  samples.push({tic, compressMs, compressedBytes: compressed.byteLength});
}
...
const summarize = selected => ({
  ...
  compressP50Ms: percentile(selected.map(row => row.compressMs), 0.5),
  compressP95Ms: percentile(selected.map(row => row.compressMs), 0.95),
  compressedP50Bytes:
...
console.log(
  `PMLE_DVR_CODEC_NODE|PASS|codec=DOOM_DFR1_RLE|version=1|tics=5250`
  ...
  + `|compress_p95_ms=${whole.compressP95Ms.toFixed(3)}`
