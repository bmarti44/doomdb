...
tag="${PMLE_FREE_RASTER_TAG:-small-generated-real-command-v1-2026-07-27}"
...
[[ "${PMLE_FREE_RASTER_EXECUTE:-NO}" == YES ]] || exit 2
...
for output in "$build_log" "$pool_log" "$install_log" "$rank_log" \
  ...
  [[ ! -e "$output" ]] || {
    printf 'small raster evidence exists: %s\n' "$output" >&2;exit 1; }
done
competing="$(ps ax -o command= | awk '
  /[r]un-wan-matrix|[r]un-oci-dvr|[r]un-oci-free-live-renderer|[r]un-oci-free-native/ {print}
')"
[[ -z "$competing" ]] || {
  printf 'small raster refuses competing OCI work:\n%s\n' "$competing" >&2
  exit 1
}

{
  "$probe/build-free-live-renderer-teavm.sh"
...
begin
  ...
  then raise_application_error(-20796,'small raster postflight');end if;
  dbms_output.put_line(
    'PMLE_FREE_RASTER_POSTFLIGHT|PASS|diagnostic_objects=0');
end;
/
SQL
  then safe=0;fi
...
begin
  ...
  if l_active<>0 or l_assigned<>0 then
    raise_application_error(-20796,'small raster requires idle capacity');
  end if;
  for slot_ in (
    ...
    doom_worker_lifecycle.stop_job(
      slot_.job_name,true,'OCI small raster diagnostic',
  ...
  dbms_output.put_line(
    'PMLE_FREE_RASTER_POOL|PASS|active_matches=0|live_slots=0');
end;
/
SQL
pool_parked=1
grep -Fq 'PMLE_FREE_RASTER_POOL|PASS|' "$pool_log"
...
"$root/scripts/adb-doom-sql.sh" \
  "$probe/benchmark-oci-free-raster-teavm.sql" | tee "$rank_log"
[[ "$(grep -c '^PMLE_FREE_RASTER_EQUIVALENCE|PASS|' "$rank_log")" == 3 ]]
[[ "$(grep -c '^PMLE_FREE_RASTER_PASS|PASS|' "$rank_log")" == 12 ]]
[[ "$(grep -c '^PMLE_FREE_RASTER_BATCH|PASS|' "$rank_log")" == 12 ]]
final_batch_ms="$(awk -F'[=|]' '
  $1=="PMLE_FREE_RASTER_BATCH" && $2=="PASS" {
    pass=0;value=0
    for(i=1;i<=NF;i++){
      if($i=="pass")pass=$(i+1)
...
verdict="$(awk -v value="$final_batch_ms" 'BEGIN{
  if(value<=8)print "PROMOTE_RETAINED_SPLIT_GENERATED_RASTER";
  else if(value>=20)print "CLOSE_LONG_INVOCATION_RASTER";
  else print "REQUIRE_INTEGRATED_VIEWPORT";
}')"
printf 'PMLE_FREE_RASTER_VERDICT|%s|final_two_worst_batch_ms_per_frame=%s|kernel=REAL_E1M1_RESOLVED_WALL_SPANS|classification=DIAGNOSTIC_NOT_GATE\n' \
  "$verdict" "$final_batch_ms" | tee "$verdict_log"
