...
tag="${PMLE_FREE_NATIVE_OVERLAY_TAG:-native-overlay-v1-2026-07-26}"
...
for output in "$build_log" "$pool_log" "$install_log" "$rank_log" \
  ...
  [[ ! -e "$output" ]] || {
    printf 'native overlay evidence exists: %s\n' "$output" >&2;exit 1; }
done
competing="$(ps ax -o command= | awk '
  /[r]un-wan-matrix|[r]un-oci-dvr|[r]un-oci-free-live-renderer[.]sh|[r]un-oci-free-live-renderer-teavm[.]sh|[r]un-oci-free-live-renderer-hybrid[.]sh/ {print}
')"
[[ -z "$competing" ]] || {
  printf 'native overlay refuses competing OCI work:\n%s\n' \
    "$competing" >&2;exit 1; }
  ...
  if [[ "$loaded" == 1 ]]; then
    {
      "$root/scripts/adb-doom-sql.sh" \
        "$probe/cleanup-free-native-overlay.sql"
      "$root/scripts/adb-doom-sql.sh" \
        "$probe/cleanup-free-live-renderer-teavm.sql"
    } >"$cleanup_log" || safe=0
  else
    : >"$cleanup_log"
  fi
...
begin
  ...
  then raise_application_error(-20796,'native overlay postflight');end if;
  dbms_output.put_line(
    'PMLE_FREE_NATIVE_OVERLAY_POSTFLIGHT|PASS|diagnostic_objects=0');
end;
/
SQL
  then safe=0;fi
...
begin
  ...
  if l_active<>0 or l_assigned<>0 then
    raise_application_error(-20796,'native overlay requires idle capacity');
  end if;
  for slot_ in (
    ...
    doom_worker_lifecycle.stop_job(
      slot_.job_name,true,'OCI native overlay diagnostic',
  ...
  dbms_output.put_line(
    'PMLE_FREE_NATIVE_OVERLAY_POOL|PASS|active_matches=0|live_slots=0');
end;
/
SQL
pool_parked=1
grep -Fq 'PMLE_FREE_NATIVE_OVERLAY_POOL|PASS|' "$pool_log"
...
loaded=1
{
  "$probe/install-free-live-renderer-teavm.sh" --emit-sql |
    "$root/scripts/adb-doom-sql.sh" -
  "$root/scripts/adb-doom-sql.sh" \
    "$probe/install-free-native-overlay.sql"
} | tee "$install_log"
grep -Fq 'PMLE_FREE_LIVE_TEAVM_STAGING|PASS|' "$install_log"
grep -Fq 'PMLE_FREE_NATIVE_OVERLAY_INSTALL|PASS|' "$install_log"
"$root/scripts/adb-doom-sql.sh" \
  "$probe/benchmark-oci-free-native-overlay.sql" | tee "$rank_log"
[[ "$(grep -c '^PMLE_FREE_NATIVE_OVERLAY_EQUIVALENCE|PASS|' "$rank_log")" == 5 ]]
[[ "$(grep -c '^PMLE_FREE_NATIVE_OVERLAY_PASS|PASS|' "$rank_log")" == 12 ]]
final_p95="$(awk -F'[=|]' '
  $1=="PMLE_FREE_NATIVE_OVERLAY_PASS" && $2=="PASS" {
    pass=0;p95=0
...
verdict="$(awk -v p95="$final_p95" 'BEGIN{
  if(p95<=20)print "PROMOTE_NATIVE_OVERLAY_TO_INTEGRATED_REQUEST";
  else if(p95>=28.571)print "REJECT_NATIVE_OVERLAY";
  else print "MEASURE_NATIVE_OVERLAY_INTEGRATED_REQUEST";
}')"
printf 'PMLE_FREE_NATIVE_OVERLAY_VERDICT|%s|final_two_worst_p95_ms=%s|kernel=GENERATED_CACHE_NATIVE_UTL_RAW_OVERLAY|classification=DIAGNOSTIC_NOT_GATE\n' \
