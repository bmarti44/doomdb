...
create or replace view doom_r1_staged_ray_rows as
select
  ...
  ray.cam_x,ray.ray_x,ray.ray_y,ray.ray_length_squared,
...
join doom_render_ray ray
 ...
 and ray.angle_degrees=player.angle
where far_config.config_key = 'FAR_DISTANCE';

create or replace function doom_r1_staged_rays(
  p_session varchar2
...
begin
  return q'~
    select ray.*
    from doom_r1_staged_ray_rows ray
...
segment_camera as (
  select segment.*,pose.direction_x,pose.direction_y,
    pose.plane_x,pose.plane_y,
    (segment.seg_start_x-pose.player_x)*pose.direction_x+
      (segment.seg_start_y-pose.player_y)*pose.direction_y start_depth,
    (segment.seg_end_x-pose.player_x)*pose.direction_x+
      (segment.seg_end_y-pose.player_y)*pose.direction_y end_depth,
    (segment.seg_start_x-pose.player_x)*pose.plane_x+
      (segment.seg_start_y-pose.player_y)*pose.plane_y start_plane,
    (segment.seg_end_x-pose.player_x)*pose.plane_x+
      (segment.seg_end_y-pose.player_y)*pose.plane_y end_plane,
    pose.plane_x*pose.plane_x+pose.plane_y*pose.plane_y plane_squared
  from segment_inputs segment
  join pose on pose.session_token=segment.session_token
),
-- Perspective projection of a segment is monotone while it is in front of
-- the camera plane.  Clip an endpoint behind the accepted t>1e-9 plane, then
-- range-join only screen-column centers covered by the conservative interval.
-- The original determinant/t/u predicates below remain the exact authority.
projected_segments as (
  select segment_camera.*,
    case when start_depth>1e-9 then
      start_plane/(start_depth*plane_squared)
    else
      (start_plane+(1e-9-start_depth)*(end_plane-start_plane)/
        nullif(end_depth-start_depth,0))/(1e-9*plane_squared)
    end start_cam_x,
    case when end_depth>1e-9 then
      end_plane/(end_depth*plane_squared)
    else
      (end_plane+(1e-9-end_depth)*(start_plane-end_plane)/
        nullif(start_depth-end_depth,0))/(1e-9*plane_squared)
    end end_cam_x
  from segment_camera
  where greatest(start_depth,end_depth)>1e-9
),
segment_bounds as (
  select projected_segments.*,
    least(start_cam_x,end_cam_x)-1e-7 min_cam_x,
    greatest(start_cam_x,end_cam_x)+1e-7 max_cam_x
  from projected_segments
),
...
classified as (
  select accepted.*,
    case
      ...
      when least(facing_sector.ceiling_height,
                 opposite_sector.ceiling_height)
           - greatest(facing_sector.floor_height,
                      opposite_sector.floor_height) <= 0 then 1
      else 0
    end as is_solid
  from accepted
...
create or replace function doom_r1_staged_hits(
  p_session varchar2
) return varchar2 sql_macro(table)
is
begin
  return q'~
    select hit.*
    from doom_r1_staged_hit_rows hit
    where hit.session_token = p_session
  ~';
end;
/

create or replace function doom_r1_staged_nearest(
  p_session varchar2
) return varchar2 sql_macro(table)
is
begin
  return q'~
    ...
    from (
      ...
      from doom_r1_staged_hit_rows solid_hit
