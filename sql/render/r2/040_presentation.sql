...
-- T5.4 owns the final database-authored 320 by 200 presentation canvas.
-- Every WAD image below is resolved through DOOM_ASSET (ASSET_KIND is either
-- UI_PATCH or SPRITE_PATCH) and AT.  AT.C >= 0 is the TRANSPARENT patch-hole
-- predicate: negative texels never enter the candidate relation, preserving
-- the immediately lower layer.  HUD/STATUS candidates intentionally outrank
-- the first-person weapon where the weapon overlaps the 168-row viewport.
--
-- Automap endpoints are derived only from DOOM_LINEDEF and DOOM_VERTEX.  The
-- browser supplies neither projected endpoints nor visibility decisions.
-- Rasterization is a bounded relational expansion and contains no procedural
-- pixel loop or dynamic SQL.
create or replace function doom_r2_presentation(
  p_session varchar2
...
begin
  return q'~
    ...
    from ranked
    where winner_ordinal=1 and session_token=p_session
