...
create or replace view doom_r2_staged_hit_geometry as
with
resolved as (
  select
    hit.session_token,
...
create or replace view doom_r2_staged_portal_hit_rows as
select
  session_token,
  column_no,
  hit_ordinal,
  ...
  case when match_class = 'ACTIVE' and is_closed_base = 1 then 1 else 0 end
    as is_termination
from doom_r2_staged_hit_geometry
match_recognize (
...
create or replace function doom_r2_staged_portal_hits(
  p_session varchar2
...
create or replace view doom_r2_staged_sector_interval_rows as
with
active_hits as (
  select hit.*,
    row_number() over (
...
create or replace function doom_r2_staged_sector_intervals(
  p_session varchar2
) return varchar2 sql_macro(table)
is
begin
