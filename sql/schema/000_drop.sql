...
begin
  ...
  begin dbms_scheduler.drop_job('DOOM_MLE_WARM_JANITOR',true); exception when others then null; end;
  for l_slot in 1..4 loop
    if l_slot<=2 then
      begin
        dbms_scheduler.drop_job(
          'DOOM_MLE_WARM_'||to_char(l_slot,'FM00'),true);
  ...
  for l_name in (
    select column_value function_name
      from table(sys.odcivarchar2list(
        ...
        'DOOM_MLE_LIVE_RENDER_PUBLISH_VIEWS',
        'DOOM_MLE_LIVE_PREPARE_VIEWS',
        'DOOM_MLE_LIVE_PUBLISH_PREPARED_VIEWS',
  ...
  for t in (
    select column_value table_name
      from table(sys.odcivarchar2list(
        ...
        'DOOM_WORKER_STOP_INTENT','DOOM_MLE_WARM_ASSIGNMENT','DOOM_MLE_WARM_LAUNCH','DOOM_MLE_PREWARM_RUN','DOOM_MLE_WARM_SLOT','DOOM_MLE_TIC0_CHECKPOINT','DOOM_MLE_LIVE_FRAME_SOURCE','DOOM_MATCH_FRAME_STAGE_WINDOW','DOOM_MATCH_LIVE_FRAME_VIEWS','DOOM_MATCH_LIVE_FRAME_BATCH','DOOM_MATCH_LIVE_FRAME','DOOM_MATCH_POLL_LEASE','DOOM_MATCH_POLL_CAPACITY','DOOM_MATCH_LIVENESS_PROBE','DOOM_MATCH_SLOW_CALL','DOOM_MATCH_TRANSITION','DOOM_MATCH_ROUTE_TRACE','DOOM_MATCH_CHECKPOINT_PROBE','DOOM_MATCH_STANDBY_CONTROL','DOOM_MATCH_WORKER_CONTROL','DOOM_MATCH_CHECKPOINT','DOOM_MATCH_FRAME','DOOM_MATCH_INPUT_EVENT','DOOM_MATCH_COMMAND','DOOM_MATCH_TIC','DOOM_MATCH_MEMBER','DOOM_MATCH',
