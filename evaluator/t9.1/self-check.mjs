...
const small=generate(f.feasibility.small),smallStats=stats(small);eq(smallStats,{cells:1536,min:0,max:36,nonzero:576,runs:485,rawBytes:1536,runRows:485},'small probe exact');eq(animationHash(small),'839e061d802ca3021f2261bb3e644b8bb9d1149847cf8c9a792467417f60295d','small probe hash');ok(e.stats.cells*32<f.feasibility.maxEstimatedBytes,'conservative full memory bound');eq(f.visualReview.status,'APPROVED','database animation visual review');eq(f.visualReview.artifactSha256,'79c27b9755741d90c75d405b3ecb01a3dd676957d5517266acffdbf9c24169b3','reviewed APNG identity');eq(f.visualReview.reviewedFrameHashes,[0,35,75,110,149].map(index=>e.frameHashes[index]),'exact reviewed frame identities');ok(f.visualReview.note.includes('database-derived')&&f.visualReview.note.includes('2026-07-19'),'visual approval provenance');
