...
export function payload(tic=0,{audio=tic?[tic,0,'DSPISTOL',255,128]:null,mode='GAME'}={}){const idx=indices(tic);return {v:1,tic,w:WIDTH,h:HEIGHT,mode,state_sha:sha(Buffer.from(`state:${tic}`)),frame_sha:sha(idx),cols:columns(tic),audio:audio?[audio]:[],complete:0}}
...
export function validateRle(p){if(p.v!==1||p.w!==WIDTH||p.h!==HEIGHT||p.cols?.length!==WIDTH)throw Error('payload dimensions');const out=Buffer.alloc(WIDTH*HEIGHT);for(let x=0;x<WIDTH;x++){let y=0;for(const run of p.cols[x]){if(!Array.isArray(run)||run.length!==3)throw Error('run shape');const [y0,n,c]=run;if(y0!==y||!Number.isInteger(n)||n<1||y+n>HEIGHT||!Number.isInteger(c)||c<0||c>255)throw Error('run value');for(let q=0;q<n;q++)out[(y+q)*WIDTH+x]=c;y+=n}if(y!==HEIGHT)throw Error('run coverage')}if(sha(out)!==p.frame_sha)throw Error('frame hash');return out}
