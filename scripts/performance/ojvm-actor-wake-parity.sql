...
declare
  ...
  los_sector_ number;player_target_ number;actor_count_ pls_integer;dirty_ pls_integer;wakes_ pls_integer;
...
begin
  ...
  select min(source_sector_id) into los_sector_ from doom_sector_reject
    where target_sector_id=player_sector_ and rejected=0;
  if los_sector_ is null then raise_application_error(-20000,'LOS fixture pair');end if;
  ...
  update mobjs m set sector_id=los_sector_,awake=0,target_mobj_id=null,
  ...
  select json_arrayagg(json_array(m.mobj_id,m.health,m.monster_health_seen,
           m.attack_cooldown,m.awake,m.state_tics,m.sector_id,cur.state_index,
  ...
  delta_:=doom_actor_wake_prepare(session_,lineage_,1,
    ...
    to_binary_double(player_y_),0,player_target_,0);
  if rawtohex(utl_raw.substr(delta_,6,1))<>'01' then
    raise_application_error(-20000,'LOS-dependent actor did not fail closed');
  end if;

  -- Restore the supported REJECT-hidden/sound-reachable snapshot.
  update mobjs m set sector_id=hidden_sector_ where m.session_token=session_
    and exists(select 1 from doom_monster_def d where d.thing_type=m.thing_type);
  with states as (
    select state_id,tics,row_number() over(order by state_id)-1 state_index from doom_state_def
  )
  ...
  delta_:=doom_actor_wake_prepare(session_,lineage_,1,request_,
    to_binary_double(player_x_),to_binary_double(player_y_),1,player_target_,1);
  ...
  for event_ in (
    ...
    offset_:=event_offset_+row_index_*12;
  ...
  dbms_output.put_line('actor_los_fail_closed=PASS');
