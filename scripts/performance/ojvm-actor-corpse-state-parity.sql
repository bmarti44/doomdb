...
declare
  ...
  result_ varchar2(4000);delta_ raw(32767);player_x_ number;player_y_ number;
  player_target_ number;rng_before_ number;rng_after_ number;actor_count_ number;
  dirty_ number;events_ number;row_index_ number:=0;offset_ number;
begin
  ...
  update mobjs m set health=0,monster_health_seen=0,attack_cooldown=0,
    awake=0,state_id=(select d.death_state_id from doom_monster_def d
      where d.thing_type=m.thing_type),state_tics=1,death_processed=1,
    sector_id=coalesce(m.sector_id,
  ...
  delta_:=doom_actor_wake_prepare(session_,lineage_,1,
    '77777777777777777777777777777777',player_x_,player_y_,0,player_target_,0);
  if rawtohex(utl_raw.substr(delta_,1,6))<>'4441574B0100' then
    raise_application_error(-20000,'corpse prepare '||doom_actor_wake_last_error);
  end if;
  dirty_:=to_number(rawtohex(utl_raw.substr(delta_,7,2)),'xxxx');
  events_:=to_number(rawtohex(utl_raw.substr(delta_,9,2)),'xxxx');
  if dirty_<>actor_count_ or events_<>0 then raise_application_error(-20000,'corpse counts');end if;
  doom_monsters.advance(session_,905);
  select rng_cursor into rng_after_ from game_sessions where session_token=session_;
  if rng_after_<>rng_before_ then raise_application_error(-20000,'corpse RNG');end if;
  for row_ in (
    with states as (
      select state_id,row_number() over(order by state_id)-1 state_index from doom_state_def
    )
    select m.mobj_id,s.state_index,m.state_tics
      from mobjs m join doom_monster_def d on d.thing_type=m.thing_type
    ...
    if utl_raw.cast_to_binary_integer(utl_raw.substr(delta_,offset_,4),utl_raw.big_endian)<>
       ...
       utl_raw.cast_to_binary_integer(utl_raw.substr(delta_,offset_+24,4),utl_raw.big_endian)<>
         row_.state_tics then raise_application_error(-20000,'corpse transition mismatch');end if;
    row_index_:=row_index_+1;
  end loop;
  result_:=doom_actor_wake_accept(session_,lineage_,1,
    '77777777777777777777777777777777');
  if result_<>'OK' then raise_application_error(-20000,result_);end if;
  dbms_output.put_line('actor_processed_corpse_transition_parity='||actor_count_||'/'||actor_count_);
  dbms_output.put_line('actor_processed_corpse_events=0/0');
  dbms_output.put_line('actor_processed_corpse_rng=PASS');
  rollback;
end;
/
