...
deployment:{tool:policy.sqlcl,onErrorExit:true,networkInstall:false,retries:0,scripts:deployment.domains,manifestSha256:sha(read(deployPath)),javaArtifact:deployment.javaArtifact},
...
catalog:{invalidObjects:0,sqlErrors:0,disabledConstraints:0,unvalidatedConstraints:0,probeObjectsRemaining:0,objectFingerprintSha256:sha([...catalog.matchAll(/^T111_OBJECT\|.*$/gm)].map(x=>x[0]).join('\n')),constraintFingerprintSha256:sha([...catalog.matchAll(/^T111_CONSTRAINT\|.*$/gm)].map(x=>x[0]).join('\n'))},
