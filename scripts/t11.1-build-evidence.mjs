...
const mle=catalog.match(/^T111_MLE\|1\|1\|27\|1090790\|([0-9a-f]{64})\|180272\|([0-9a-f]{64})\|([0-9a-f]{64})$/m);
...
const api=JSON.parse(apiText.trim());assert.equal(api.observations.length,20);
...
catalog:{invalidObjects:0,sqlErrors:0,disabledConstraints:0,unvalidatedConstraints:0,probeObjectsRemaining:0,javaObjects:0,javaCallSpecs:0,javaDependencies:0,legacyOjvmObjects:0,legacyApiProcedures:0,mleCallSpecs:27,objectFingerprintSha256:sha([...catalog.matchAll(/^T111_OBJECT\|.*$/gm)].map(x=>x[0]).join('\n')),constraintFingerprintSha256:sha([...catalog.matchAll(/^T111_CONSTRAINT\|.*$/gm)].map(x=>x[0]).join('\n'))},
