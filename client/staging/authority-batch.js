import { base64AuthorityBytes, commitAuthorityTransition, decodeAuthorityTransitionBytes } from './authority.js';
 ...
 */
export async function decodeAuthorityBatch(encoded, state) {
    ...
    for (let index = 0; index < count; index += 1) {
        if (offset > bytes.length - 4)
            throw new TypeError('authority batch is truncated');
        ...
        const transition = await decodeAuthorityTransitionBytes(bytes.slice(offset, offset + length), cursor);
        if (transition.generation > generation) {
            throw new TypeError('authority batch generation changed');
        }
        transitions.push(transition);
        commitAuthorityTransition(cursor, transition);
        offset += length;
    }
