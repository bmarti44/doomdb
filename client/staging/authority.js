...
export function base64AuthorityBytes(encoded) {
...
function audioTuples(value, tic) {
    if (!Array.isArray(value))
        throw new TypeError('authority audio is invalid');
    return value.map((entry, ordinal) => {
        ...
        if (eventTic !== tic || eventOrdinal !== ordinal || typeof asset !== 'string' ||
            ...
            !Number.isInteger(separation) || volume < 0 ||
            volume > 255 || separation < 0 ||
            separation > 255) {
            throw new TypeError('authority audio tuple is invalid');
        }
...
async function chainDigest(bytes) {
...
export async function authorityRootChainSha(match, membershipEpoch) {
    if (!/^[0-9a-f]{32}$/.test(match) || !Number.isSafeInteger(membershipEpoch) ||
        membershipEpoch < 1) {
        throw new TypeError('authority root fence is invalid');
    }
    return hex(new Uint8Array(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(`DMD1_ROOT|${match}|${membershipEpoch}`))));
}
/**
 ...
 */
export async function decodeAuthorityTransition(encoded, state) {
...
export async function decodeAuthorityTransitionBytes(bytes, state) {
    ...
    const membershipBitmap = bytes[16];
    const activePlayers = bytes[17];
...
export function commitAuthorityTransition(state, transition) {
