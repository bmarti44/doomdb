...
const requiresCameraPixelResponse = (previous, current) => previous === null
    || previous.turn !== current.turn
    || previous.forward !== current.forward
    || previous.strafe !== current.strafe
    || previous.run !== current.run;
...
async function startDatabaseFrameGame(value, status) {
    ...
    const postInput = () => {
        ...
        if (soloMode && pixelPollInFlight.size > 0
            && performance.now() - pixelPollStartedAt < 75)
            return;
        ...
        const fusedCameraResponse = soloMode && requiresCameraPixelResponse(lastEffectiveCommand, input.command);
        const operation = fusedCameraResponse
            ? exchangeMatchPixelBatch(value.match, value.playerCapability, transportTic, 1, input.sequence, input.hex, input.targetTic, 225).then(async (result) => {
                ...
                const decoded = await decodeDatabasePixelTransport(decodeBytes(result.payload), value.playerSlot);
            ...
            : reviseMatchInput(value.match, value.playerCapability, input.sequence, input.hex, input.targetTic);
        void operation.then(result => {
            ...
            if (playoutStarted && changedInput && presentationCatchup) {
                ...
                const confirmedCatchupTic = confirmedInputCatchupCursor(presentedTic, result.effectiveTic, transportTic);
                ...
                const catchupTic = soloMode ? confirmedCatchupTic :
                    Math.max(presentedTic, confirmedCatchupTic - pixelInputCatchupFloor);
    ...
    const pump = () => {
        ...
        if (frame === undefined) {
            if (playoutStarted && now >= nextFrameAt +
                databasePixelPlayoutIntervalMs(soloMode, 'FREE', false) / 2 && !starvationActive) {
        ...
        if (!playoutStarted) {
            ...
            const startupFrames = Math.min(128, wan.playoutBufferTics + wan.expectedConfirmedBatchTics);
    ...
    const launchPixelPoll = (expectedTic) => {
        ...
        void exchangeMatchPixelBatch(value.match, value.playerCapability, requestAfterTic, 8, undefined, undefined, undefined, soloSeekRequest ? 225 : 0)
            .then(async (result) => {
            ...
            if (result.frameCount > 0 && result.firstTic !== null
                ...
                if (frames.size > 64) {
                    ...
                    const retain = Math.min(128, wan.playoutBufferTics + wan.expectedConfirmedBatchTics + 2);
