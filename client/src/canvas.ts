...
export function createIndexedBlitter(
    canvas: HTMLCanvasElement, palette: Uint8Array<ArrayBuffer>):
    (indices: Uint8Array<ArrayBuffer>) => void {
  if (palette.length !== 256 * 3) throw new TypeError('palette byte length is invalid');
  const context = canvas.getContext('2d', {willReadFrequently: true});
  if (context === null) throw new Error('canvas context is unavailable');
  context.imageSmoothingEnabled = false;
 ...
 */
export function createColumnMajorIndexedBlitter(
    canvas: HTMLCanvasElement, palette: Uint8Array<ArrayBuffer>):
    (indices: Uint8Array<ArrayBuffer>) => void {
  if (palette.length !== 256 * 3) throw new TypeError('palette byte length is invalid');
  const context = canvas.getContext('2d', {willReadFrequently: true});
  ...
  const rgbaWords = new Uint32Array(
    image.data.buffer,image.data.byteOffset,image.data.byteLength/4);
...
export function createColumnMajorIndexedPaletteBlitter(
    canvas: HTMLCanvasElement, palettes: Uint8Array<ArrayBuffer>):
    (indices: Uint8Array<ArrayBuffer>,paletteIndex:number) => void {
  ...
  for (let index=0;index<14*256;index+=1) {
    const source=index*3;
    const red=palettes[source]!,green=palettes[source+1]!,
      blue=palettes[source+2]!;
    paletteWords[index]=littleEndian
      ? red|(green<<8)|(blue<<16)|0xff000000
      : (red<<24)|(green<<16)|(blue<<8)|0xff;
  }
...
export function createDatabaseIndexedPaletteBlitter(
    canvas: HTMLCanvasElement, palettes: Uint8Array<ArrayBuffer>):
    (indices: Uint8Array<ArrayBuffer>,paletteIndex:number,
      layout:'COLUMN_MAJOR'|'ROW_MAJOR') => void {
  if (palettes.length !== 14 * 256 * 3)
    throw new TypeError('PLAYPAL set byte length is invalid');
  const context = canvas.getContext('2d', {willReadFrequently: true});
  if (context === null) throw new Error('canvas context is unavailable');
