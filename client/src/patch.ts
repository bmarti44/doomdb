...
export type DoomPatch = {
  width: number;
  height: number;
  leftOffset: number;
  topOffset: number;
  pixels: Int16Array<ArrayBuffer>;
};

export function decodePatch(payload: string): DoomPatch {
  const bytes = decodeBytes(payload);
  if (bytes.length < 12) throw new TypeError('Doom patch is truncated');
  ...
  if (width < 1 || height < 1 || width > 320 || height > 200 ||
      8 + width * 4 > bytes.length) throw new TypeError('Doom patch header is invalid');
  ...
  for (let x = 0; x < width; x += 1) {
    ...
    for (;;) {
      ...
      if (rawTop === 255) break;
      if (source + 4 > bytes.length) throw new TypeError('Doom patch post is truncated');
      ...
      for (let y = 0; y < length; y += 1) {
        if (top + y >= 0 && top + y < height) pixels[(top + y) * width + x] = bytes[source + y]!;
      }
      source += length + 1;
    }
  }
...
export function drawPatch(target: Uint8Array<ArrayBuffer>, patch: DoomPatch,
                          x: number, y: number): void {
  if (target.length !== 320 * 200) throw new TypeError('menu target is invalid');
  const originX = x - patch.leftOffset;
  const originY = y - patch.topOffset;
  for (let py = 0; py < patch.height; py += 1) {
    const targetY = originY + py;
    if (targetY < 0 || targetY >= 200) continue;
    for (let px = 0; px < patch.width; px += 1) {
      const targetX = originX + px;
      const color = patch.pixels[py * patch.width + px]!;
      if (color >= 0 && targetX >= 0 && targetX < 320) target[targetY * 320 + targetX] = color;
    }
  }
}
