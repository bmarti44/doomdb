import {commitAuthorityTransition, type AuthorityStreamState,
  type AuthorityTransition} from './authority.js';

export type TeaVmAuthorityVerifier = {
  stepMultiplayerAuthoritative(activePlayers: number, membershipMask: number,
    commands: Uint8Array<ArrayBuffer>): number;
  canonicalState(): string;
  canonicalStateLength(): number;
  canonicalStateChunk(offset: number, length: number): Uint8Array<ArrayBuffer>;
};

export type TeaVmAuthorityPresenter = {
  stepMultiplayerAuthoritative(activePlayers: number, membershipMask: number,
    commands: Uint8Array<ArrayBuffer>): number;
  renderPlayerFrame(playerSlot: number): Uint8Array<ArrayBuffer>;
};

export type ConfirmedPresentation = {
  tic: number;
  chainSha: string;
...
async function canonicalSha(engine: TeaVmAuthorityVerifier): Promise<string> {
  ...
  return hex(new Uint8Array(await crypto.subtle.digest(
    'SHA-256', new TextEncoder().encode(record))));
}
...
export class ConfirmedAuthorityMirror {
  readonly #verifier: TeaVmAuthorityVerifier;
  readonly #presenter: TeaVmAuthorityPresenter;
  readonly #playerSlot: number;
  readonly #state: AuthorityStreamState;
  #failed = false;

  constructor(verifier: TeaVmAuthorityVerifier, presenter: TeaVmAuthorityPresenter,
              playerSlot: number,
              initialState: AuthorityStreamState) {
    if (!Number.isInteger(playerSlot) || playerSlot < 0 || playerSlot > 3) {
      throw new TypeError('authority mirror player slot is invalid');
    }
    if ((verifier as object) === (presenter as object)) {
      throw new TypeError('authority verifier and presenter must be independent');
    }
    this.#verifier = verifier;this.#presenter = presenter;
    this.#playerSlot = playerSlot;
    this.#state = {...initialState};
  }

  get frontier(): Readonly<AuthorityStreamState> { return {...this.#state}; }

  async apply(transition: AuthorityTransition): Promise<ConfirmedPresentation> {
    if (this.#failed) throw new TypeError('authority mirror requires recovery');
    ...
    const tic = this.#verifier.stepMultiplayerAuthoritative(
      transition.activePlayers, transition.membershipBitmap, transition.commands);
    try {
      ...
      const presentationTic = this.#presenter.stepMultiplayerAuthoritative(
        transition.activePlayers, transition.membershipBitmap, transition.commands);
