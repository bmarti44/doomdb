...
let retainedStatusStates = [undefined, undefined, undefined, undefined];
const STATUS_STATE_OFFSETS = new Int32Array([
  56, 60, 64, 72, 76, 80, 84, 144, 148, 128, 132, 168, 180,
]);
...
function clearRetainedWorldFrames() {
  ...
  retainedStatusStates = [undefined, undefined, undefined, undefined];
}
...
function statusStateUnchanged(playerSlot, snapshot) {
  let state = retainedStatusStates[playerSlot];
  if (!(state instanceof Int32Array) || state.length !== 14) {
    state = new Int32Array(14);
    state.fill(-0x80000000);
    retainedStatusStates[playerSlot] = state;
  }
  const health = Math.max(0, snapshotI32(snapshot, 56));
  const damage = snapshotI32(snapshot, 128);
  const bonus = snapshotI32(snapshot, 132);
  const cheats = snapshotI32(snapshot, 168);
  const refire = snapshotI32(snapshot, 180);
  const tic = snapshotI32(snapshot, 8);
  let face;
  if (health <= 0) face = 1;
  ...
  else face = 8 + Math.floor(tic / 17) % 3;
  let unchanged = state[0] === face;
  state[0] = face;
  for (let index = 0; index < STATUS_STATE_OFFSETS.length; index++) {
    const value = snapshotI32(snapshot, STATUS_STATE_OFFSETS[index]);
    if (state[index + 1] !== value) unchanged = false;
    state[index + 1] = value;
  }
  return unchanged;
}
...
function renderCompleteMatchFrame(
    ...
    retainPlayerStatus = false, priorStatusOffset = -1) {
  if (fullWorld) {
  ...
  copyCurrentCompositionBuffers();
  if (retainPlayerStatus && priorStatusOffset >= 0) {
    for (let x = 0; x < FRAME_WIDTH; x++) {
      const sourceAt = priorStatusOffset + x * FRAME_HEIGHT + 168;
      retainedFrame.set(
        retainedMatchViews.subarray(sourceAt, sourceAt + 32),
        x * FRAME_HEIGHT + 168);
    }
  }
  composePreparedWorldSprites();
  composePreparedWeapon();
  if (!retainPlayerStatus
      || !statusStateUnchanged(playerSlot, compositorSnapshot)) {
...
export function renderAndPublishMatchViews(
  ...
  for (let playerSlot = 0; playerSlot < 2; playerSlot++) {
    ...
    const priorStatusOffset = changed
      ? -1 : MATCH_VIEW_HEADER_BYTES + playerSlot * FRAME_BYTES;
    renderCompleteMatchFrame(
      playerSlot, changed && first, frameTic, true, priorStatusOffset);
