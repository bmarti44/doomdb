...
from doom_r1_hit_rows
where session_token='12001200120012001200120012001200';

column stage format a20
select 'STAGED_HITS' stage,count(*) rows_,sum(hit_ordinal) ordinal_sum,
  round(sum(hit_t),6) depth_sum from frame_r1_hit;

delete from frame_world_pixel;
insert into frame_world_pixel
select /*+ opt_param('optimizer_adaptive_plans' 'false') opt_param('_optimizer_use_feedback' 'false') */ *
from doom_r2_staged_pixel_rows
where session_token='12001200120012001200120012001200';

select 'WORLD' stage,count(*) rows_,sum(palette_index) palette_sum,
  sum(column_no*1000+row_no) coord_sum from frame_world_pixel;
...
where session_token='12001200120012001200120012001200' and is_selected=1;

select 'MASKED_SELECTED' stage,count(*) rows_,sum(palette_index) palette_sum,
  sum(column_no*1000+row_no) coord_sum from frame_masked_pixel;

delete from frame_column;
insert into frame_column(session_token,column_no)
select '12001200120012001200120012001200',level-1
from dual connect by level<=320;

select /*+ opt_param('optimizer_adaptive_plans' 'false') opt_param('_optimizer_use_feedback' 'false') */
  'PRESENTATION' stage,count(*) rows_,sum(palette_index) palette_sum,
  sum(column_no*1000+row_no) coord_sum
from doom_api_presentation_rows
where session_token='12001200120012001200120012001200';
